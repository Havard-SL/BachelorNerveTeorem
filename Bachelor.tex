...
\begin{proof} % Å herregud ditta beviset e vanskelegare enn eg trudde. % Shit, blande nokre bilete her. må dirrenetsiera mellom pre og post translasjon av mangder. Lineær operator definert av kor den sender n lineært uavhengige vektorar.
	...
	\( \hat{L} \) er surjektiv fordi gitt ein \( y \in \gr{K}_{\hat{g}} \), så er det ein konveks kombinasjon av punkt \( \hat{g}(\sigma) \) for \( \sigma \in K \). Vel ein ordning av hjørnene: \( \sigma = (v_1, v_2, \dots, v_{\#\sigma}) \), og la \( y \) ha barysentrisk koordinatar \( (a_1, a_2, \dots, a_{\#\sigma}) \)
	\[
		y = \sum_{i = 1}^{\#\sigma} a_i \hat{g}(v_i)
	\]
	Anta utan tap av generalitet at ordninga av hjørnene i \( \sigma \) gjer at \( a_1 \geq a_2 \geq \dots \geq a_{\#\sigma} \) for \( y \).
