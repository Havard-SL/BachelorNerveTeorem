...
\begin{proof} % Å herregud ditta beviset e vanskelegare enn eg trudde. % Shit, blande nokre bilete her. må dirrenetsiera mellom pre og post translasjon av mangder. Lineær operator definert av kor den sender n lineært uavhengige vektorar.
	...
	Frå den universale eigenskapen av underromstopologien ref:TODO så får me at \( \hat{L} \) er kontinuerlig.

	\( \hat{L} \) er surjektiv fordi gitt ein \( y \in \gr{K}_{\hat{g}} \), så er det ein konveks kombinasjon av punkt \( \hat{g}(\epsilon) \) for \( \epsilon \in K \). Vel ein ordning av hjørnene: \( \epsilon = (v_1, v_2, \dots, v_{\#\epsilon}) \), og la \( y \) ha barysentrisk koordinatar \( (a_1, a_2, \dots, a_{\#\epsilon}) \)
	\[
		y = \sum_{i = 1}^{\#\epsilon} a_i \hat{g}(v_i)
	\]
	Anta utan tap av generalitet at ordninga av hjørnene i \( \epsilon \) gjer at \( a_1 \geq a_2 \geq \dots \geq a_{\#\epsilon} \) for \( y \). La 
	\begin{align*}
		(b_1, b_2, b_3, \dots, b_{\#\epsilon}) &= \left( a_1-a_2, 2(a_2-a_3), \dots, (\#\epsilon-1)(a_{\epsilon-1}-a_{\epsilon}), \#\epsilon a_{\epsilon} \right) \\
		&= \left( i (a_i-a_{i+1}) \right)_{i=1}^{\#\epsilon}, a_{\epsilon+1} = 0
	\end{align*}
	...
	\begin{align*} % Meir detaljar rundt sum manipulasjon på linja 4-5?
		\hat{L}\left(x\right) &= \hat{L}\left( \sum_{i=1}^{\#\sigma} b_i \hat{f}(\sigma_i) \right) \\
		&= \sum_{i=1}^{\#\sigma} \hat{L} \left( b_i \hat{f}(\sigma_i) \right) \\
		&= \sum_{i=1}^{\#\sigma} b_i \hat{L} \left( \hat{f}(\sigma_i) \right) \\
		&= \sum_{i=1}^{\#\sigma} b_i \frac{1}{\#\sigma_i} \sum_{v_j \in \sigma_i} \hat{g}(v_j) \\
		&= \sum_{i=1}^{\#\sigma} b_i \frac{1}{i} \sum_{j = 1}^{i} \hat{g}(v_j) \\
		\intertext{Hugs \( a_{\epsilon+1} = 0 \)}
		&= \sum_{i=1}^{\#\sigma} (a_{i}-a_{i+1}) \sum_{j = 1}^{i} \hat{g}(v_j) \\
		&= \sum_{j=1}^{\#\sigma} \hat{g}(v_j) \sum_{i = j}^{\#\sigma} (a_{i}-a_{i+1}) \\
		&= \sum_{j=1}^{\#\epsilon} a_i \hat{g}(v_j) \\
		&= y
	\end{align*}
	Fordi dei barysentriske koordinatene er unike \ref{thm:unik-barysentrisk-koordinat}. Og ettersom dette var ein vilkårleg \( y \), så er \( \hat{L} \) surjektiv.
