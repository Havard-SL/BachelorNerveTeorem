...
\begin{proof} % Å herregud ditta beviset e vanskelegare enn eg trudde. % Shit, blande nokre bilete her. må dirrenetsiera mellom pre og post translasjon av mangder. Lineær operator definert av kor den sender n lineært uavhengige vektorar.
	...
	\begin{align*} % Meir detaljar rundt sum manipulasjon på linja 4-5?
		...
		&= y
	\end{align*}
	Fordi dei barysentriske koordinatene er unike \autoref{thm:unik-barysentrisk-koordinat}. Og ettersom dette var ein vilkårleg \( y \), så er \( \hat{L} \) surjektiv.

	\( \hat{L} \) er injektiv fordi for \( x, y \in \gr{\Sd(K)}_{\hat{f}} \), la \( x \) vera ein konveks kombinasjon av punkta \( \hat{f}(\hat{\sigma}) \), for ein \( \hat{\sigma} \in \Sd(K) \) og \( y \) vera ein konveks kombnasjon av punkta \( \hat{f}(\bar{\sigma}) \), for ein \( \bar{\sigma} \in \Sd(K) \).
...
\begin{lemma} % Må visa veldefinert mhp f og phi?
	\( \Gamma \) frå \autoref{thm:Gamma} er kontinuerleg og biletet er i \( \union_{u\in U} u \).
\end{lemma}
