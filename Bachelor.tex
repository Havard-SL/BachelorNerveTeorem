...
\begin{proof} % Å herregud ditta beviset e vanskelegare enn eg trudde. % Shit, blande nokre bilete her. må dirrenetsiera mellom pre og post translasjon av mangder.
	Dette beviset bruker mykje den same strategien som i ref:TODO.

	Fyrst, lag ei ordning av elementa i \( V = (v_1, v_2, \dots, v_n) \) la \( \tau_f(x) := x-f(\set{v_1}) \), og la \( \tau_g(x) := x-g(\set{v_1}) \). Desse er homeomorfiar frå \ref{thm:tau-homeomorfi}.
