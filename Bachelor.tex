...
\begin{theorem} \label{thm:definer-lin-op}
	For ei mengde med \( n \) lineært uavhengige vektorar i \( \Rb^d \), med \( n \leq d \): \( \set{v_1, v_2, \dots, v_n} \), så kan ein konstruera ei lineæravbildning \( L \) som tek \( v_1 \mapsto L(v_1) \).
\end{theorem}

\begin{theorem} \label{thm:universal-eigenskap-underromstopologi}
	(Universaleigenskapen til underromstopologien)

	For \( X, Y \) topologiske rom, og \( A \subseteq X \) eit underrom av \( X \) med underromstopologien, og \( f: Y \to A \) ei vilkårleg avbildning frå \( Y \) til \( A \), og \( i: A \hookrightarrow X \) inklusjonsavbildninga. Då er \( f \) kontinuerlig  \( \iff i \circ f \) er kontinuerlig.
\end{theorem}
...
\begin{theorem} \label{thm:pasting-lemma}
	La \( A \) og \( B \) vera lukka delmengder underrom av eit topologisk rom \( X \) sånn at \( A \union B = X \). 
...
\begin{theorem} \label{thm:infimum-over-kompakt-er-min} \label{thm:supremum-over-kompakt-er-maks}
	La \( K \) vera ei kompakt mengde, og la \( f: K \to \Rb \) vera ein vilkårleg kontinuelrleg funskjon. Då er:
...
\begin{theorem} \label{thm:endeleg-union-kompakt-er-kompakt}
	La \( \set{K_i}_{i = 1}^n \) vera ei endeleg mengde av kompakte mengder. Då er:
	\[
		K = \union_{i=1}^n K_i
	\]
	Også kompakt.
...
\begin{definition} \label{def:ASK}
	For $V$ ei ikkje-tom og endeleg mengde, så definerar me $K$ til å vera eit \emph{endeleg asbtrakt simplisielt kompleks over $V$} om:
	\begin{enumerate}
		\item{$\forall v \in V: \{v\} \in K$}
		\item{\( \forall \sigma \in K: \sigma \subseteq V \)}
		\item{$\forall \sigma \in K, \forall \tau \subseteq \sigma: \tau \in K$}
	\end{enumerate}
	Då kallar me og $V$ for \emph{hjørnemengde} til $K$.
\end{definition}
...
\begin{definition}
	For \( K \) eit abstrakt simplisielt over \( V \), så kallar me eit element \( \sigma \in K \), med \( \#\sigma = n \) for eit \emph{abstrakt \( n-1 \)-simpleks}.
\end{definition}
...
\begin{definition}
	Ein endeleg mengde punkt $\{p_1, p_2, p_3, \dots, p_n\}$ i $\Rb^m$ for $m\geq1$ er \emph{geometrisk uavhengige} om for $\{a_i\}_{i=0}^n$ med $a_i\in\Rb$, så:
	\begin{equation*}
...
\begin{definition}
	Ein \emph{konveks kombinasjon} av ei endeleg mengde \( P = \set{p_1, p_2, p_3, \dots, p_n} \) av punkt i \( \Rb^m \) er eit punkt \( x\in\Rb^m \) gitt ein tupel av koeffisientar \( A=\{a_1, a_2, a_3, \dots, a_n\} \) i \( \Rb \), med \( a_i\geq0\forall i \) og \( \sum_{i=1}^n a_i = 1 : \)
	
	\begin{equation*}
		x=\sum_{i=1}^n a_ip_i
	\end{equation*}
	Om $P$ er ein tupel (ein ordna mengde) så blir koeffisientane også ein tupel, og $A$ blir då kalla dei \emph{barysentriske koordinatane} til $x$.
\end{definition}
...
\begin{example}
	 I \autoref{fig:tre-geometriske-simpleksar} så ser ein tre geometriske simpleksar. Ein \( 1 \)-simpleks utspunne av \( \set{a_1, a_2} \), Ein \(2\)-simpleks utspunne av \( \set{b_1, b_2, b_3} \), og ein \(3\)-simpleks utspunne av \( \set{c_1, c_2, c_3, c_4} \)
	\begin{figure}[htbp]
		\begin{center}
...
\begin{proof}
	...
	For eit vilkårleg element $x\in\gr{K}_{\hat{f}}$ så er $x$ eit element av ein simpleks i det geometriske simplisielle komplekset. Det vil seie, me kan utrykka $x$ som ein konveks sum av ein geometrisk uavhengig mengde, som korresponderar til den simpleksen $x$ er eit element i.
	
	Me velger derfor ein $\sigma\in K$ sånn at $\hat{f}(\sigma)=\{\hat{f}(k_1), \hat{f}(k_2), \dots, \hat{f}(k_r)\}$. Dette er ein geometrisk uavhengig mengde fordi $\tau_f$ er injektiv og sender $x_i\mapsto x_i$ som er lineært uavhengig og biletet er derfor geometrisk uavhengig om $\{x_i\}$ er lineært uavhengig, som det er frå \autoref{thm:geometrisklineærtuavhengig}.
...
%	\end{center}
	...
	Merk her at avbildninga på nederste rad også kontinuerlige frå \autoref{thm:universal-eigenskap-underromstopologi}. I tillegg så får me frå \autoref{thm:tau-homeomorfi} at \( \hat{\tau}_f \) og \( \hat{\tau}_g \) er homeomorfiar.
	...
	Så me får ein avbildning: $(\hat{\tau}_g)^{-1}\circ\hat{L}\circ\hat{\tau}_f:\gr{K}_f\to\gr{K}_g$ er ein samansetting av tre homeomorfiar, og er derfor ein homeomorfi sjølve.
\end{proof}
...
\begin{definition}
	La $X$ vera ei mengde. Vidare la $F$ vera ei mengde av delmengder av $X$. \emph{Nerva} til $F$, betegna som $\Nc(F)$, er:
	\begin{equation*}
...
\begin{theorem}
	La \( U = \set{U_i}_{i=1}^n \) vera ei endeleg mengde av kompakte og konvekse delmengder av \( \Rb^d \). Då er \( \union_{u \in U} u \) homotopiekvivalent med \( \gr{\Nc(U)} \).
\end{theorem}
...
\begin{proof}
	Me ser på eit vilkårleg gemetrisk \( n \)-simpleks i \( \Rb^n \) med hjørner i \( \tuple{0, e_1, e_2, \dots, e_n} \), der \( \tuple{e_1, e_2, \dots, e_n} \) er standardbasisen til \( \Rb^n \). Dette er ein geometrisk uavhengig mengde frå \autoref{thm:geometrisklineærtuavhengig} sidan \( \tuple{e_1, e_2, \dots, e_n} \) er lineært uavhengig per definisjon. Me kallar denne \( n \)-simpleksen for \( \Delta^n \)

	\( \Delta^n \) er lukka i \( \Rb^n \) sidan om ein 
	La \( U_i \) vera hyperplanet definert av å gå gjønom dei \( n \) punkta \( \tuple{0, e_1, \dots, e_{i-1}, e_{i+1}, \dots, e_n} \), og så ser ein på halvplanet danna av den delen av \( \Rb^n \setminus U_i \) som ikkje inneheld \( e_i \) (For \( i=1 \), så ser ein på planet der ein fjernar punktet \( 0 \).). Dette kallar me for \( \hat{U}_i \). Dette er ei open mengda i \( \Rb^n \), sidan det er ein homeomorfi frå \( \Rb^n \) til \( \Rb^n \), som sender \( \hat{U}_i \) til det standard øvre hyperplanet \( H^n := (0, \infty) \times \Rb^{n-1} \).
	...
	Det andre tilfellet er at \( x \) har kun positive koordinatar, men er framlesi ikkje i \( \Delta^n \). Me ser på \( x \) som ein konveks kombinasjon av punkta \( \tuple{0, e_1, \dots, e_{i-1}, e_{i+1}, \dots, e_n} \), og skriv (med \( e_0 = 0 \)):
	\[
		x = \sum_{i=0}^n a_i e_i
	\]
	Me ser at sidan \( e_0 = 0 \), så er \( x \in \Delta^n \iff \sum_{i=1}^n a_i \leq 1 \) og \( a_i \geq 0 \forall i \). Så einaste tilfellet \( x \not\in \Delta^n \) med kun positive koordinatar er når
	\[
		\sum_{i=1}^n a_i > 1
	\]
	Men dette betyr at \( x \in \hat{U}_0 \) fordi \( \hat{U}_0 \) er hyperhalvplanet danna av punkta \( \tuple{e_1, e_2, \dots, e_n} \) som ikkje inneheld \( 0 \). Men me ser at for \( x \in U_i \), så er \( \sum_{i=1}^n a_i = 1 \), så \( \hat{U}_0 \) inneheld derfor planet danna av alle punkta med positve koordinatar og me får derfor at:
	\[
		hat{U}_0 = \set{ x : \sum_{i=1}^n a_i > 1}
	\]
	Som er nøyaktig det me kom fram til ovanfor.

	Derfor så får me at for \( x \not\in \Delta^n \implies x \in \hat{U} \).

	Vidare for \( x \in \Delta^n \) så kan me skriva \( x \) som ein konveks kombinasjon:
	\[
		x = \sum_{i=0}^n a_i e_i
	\]
	...
	Vidare så får me frå det same argumentet som over at \( x \in \Delta^n \iff \sum_{i=1}^n a_i \leq 1 \) og \( a_i \geq 0 \, \forall i \). Men dette betyr at \( x \) er ikkje i \( \hat{U}_0 \) sidan argumentet over sa at:
	\[
		\hat{U}_0 = \set{ x : \sum_{i=1}^n a_i > 1}
	\]
	Så er ikkje \( x \in \hat{U}_0 \)

	Så me får at \( x \in \Delta^n \implies x \not\in \hat{U} \).

	Samlar me saman dei to resultata så får me at \( \Rb^n \setminus \hat{U} = \Delta^n \). Og sidan \( \hat{U} \) er ein union av opne mengder, så er den open sjølve. Det gir oss at \( \Delta^n \) er lukka.

	Vidare, sidan alle hjørnene til \( \Delta^n \) er inne i den lukka ballen \( \bar{B}_{1}(0) \), som er konveks, så gir \autoref{thm:konveks-kombinasjon-i-konveks} at \( \Delta^n \in \bar{B}_{1}(0) \), så den er både begrensa og lukka. \autoref{thm:heine-borel} gir oss derfor at \( \Delta^n \) er kompakt.
	...
	\autoref{thm:heine-borel} gir oss då at \( \hat{\Delta} \) er derfor både lukka og kompakt i \( \Rb^d \).
...
\begin{proof}
	...
	Då er \( x = a_1 p_1 + a_2 p_2 \), med \( a_1 + a_2 = 1 \). Men då er \( a_2 = 1 - a_1 \), så me kan skriva: \( x = a_1 p_1 + (1-a_1) p_2 \). Men dette er jo definisjonen på eit punkt på eit linjestykke mellom \( p_1 \) og \( p_2 \). Så frå definisjonen av ei konveks mengde, så må \( x \in K \).
...
\begin{definition} \label{thm:Gamma}
	Gitt \( U = \set{U_i}_{i=1}^n \) ei endeleg mengde av konvekse mengder i \( \Rb^m \). Vidare, for kvar \( \epsilon \in \Nc(U) \) velg punkt \( v_\epsilon \in \intersect_{u \in \sigma} u \).
...
\begin{proof}
	La \( A \) vere mengde av alle mogelge delmengder av \( U \), og \( A^\epsilon \) vere alle moglege delmengder av \( U^\epsilon \). Då ser me at \( f \) dannar ein bijeksjon frå \( A \) til \( A^\epsilon \). Og sidan \( i: \Nc(U) \hookrightarrow A \), inklusjonsavbildninga, er injektiv, så er \( f \circ i \) injektiv.
	...
	Definer vidare \( D_J := \bar{B}_{4r_J}(0) \). Sidan dette er ein lukka og begrensa mengde i \( \Rb^d \), så er den kompakt frå \autoref{thm:heine-borel}.
...
\begin{definition} \label{def:psi}
	For ei endeleg mengde av kompakte delmengder \( U_i \subseteq \Rb^d \), med \( U = \set{U_i}_{i=1}^n \), velg ein \( \epsilon \) som i \autoref{thm:epsilondekke}, kan då danna:
	\[
...
\begin{definition} \label{def:Psi}
	For ei endeleg mengde av lukka, kompakte delmengder \( U_i \subseteq \Rb^d \), med \( U = \set{U_i}_{i=1}^n \). Vidare la \( \psi_i \) vera som i \autoref{def:psi}, og \( \gr{\Nc(U)}_f \) ein geometrisk realisering av nerva til \( U \) med hensyn til den affine imbeddinga \( f \). Definer for \( x \in \union_{u \in U} u \):
	\[
...
\begin{example}
	Den barysentriske stjerna er nøyaktig dei farga delane me såg i \autoref{fig:Gamma} over. Merk her at \( \Gamma \) ser ut til å alltid ta ein barysentrisk stjerne fullstendig inni eine mengda i biletet. Det blir viktig i beviset av Nerveteoremet.
\end{example}
...
\begin{proof}
	...
	\] 
	Men då er jo \( v_w \) (frå \autoref{thm:Gamma}) \( \in \intersect_{u \in w} u \subseteq U_i \) for alle \( w \). Og sidan \( \Gamma \) tek eit element, \( x \), i \( \gr{\bst(U_i)}_f \) som er ein konveks kombinasjon av punkta \( \set{f(w)}_{w \in W} \) til ein konveks kombinasjon av \( \set{v_w}_{w \in W} \subseteq U_i \), så får me at \( \Gamma(x) \) er ein konveks kombinasjon av punkta \( \set{v_w}_{w \in W} \), som alle ligg i ei konveks mengde \( U_i \), og me får derfor frå \autoref{thm:konveks-kombinasjon-i-konveks} at \( x \in U_i \).
\end{proof}
...
\begin{proof}
	...
	\begin{align*}
		...
		&= 1 - t + t = 1
	\end{align*}
	Så alle linjar mellom to vilkårlege punkt i \( \Delta \) er i \( \Delta \), og \( \Delta \) er derfor konveks.
\end{proof}
...
\begin{lemma} \label{thm:utvida-funk}
	For \( K \) eit endeleg abstrakt simplisielt kompleks over hjørnemengda \( V \), og \( \gr{K}_f \) ei geometrisk realisering av \( K \). For \( \sigma \in K \), la \( \gr{\sigma}_f \) betegna det geometriske simplekset utspunne av punkta \( f(\sigma) \) (som i \autoref{rem:gr-SK}), og \( \gr{\partial\sigma}_f := \union_{\tau \in \partial\sigma} \gr{\tau}_f \). Då er:
	\[
		\gr{\partial\sigma}_f \times I \union \gr{\sigma}_f \times \set{0, 1}
	\] 
...
\begin{lemma} \label{thm:snitt-av-ASK-er-ASK}
	For \( K_1 \) og \( K_2 \) to abstrakte simplisielle kompleks over \( V_1 \) og \( V_2 \) henholdsvis, så er \( K := K_1 \intersect K_2 \) også eit abstrakt simplisielle kompleks over \( V := V_1 \intersect V_2 \). 
\end{lemma}

\begin{proof}
	Me må visa dei tre eigenskapane i \autoref{def:ASK}:
	\begin{enumerate}
...
\begin{definition}
	La \( U \) vere eit overdekke av \( X \). Om vilkårlege endelege snitt av element av \( U \) er samantrekkbare, då kallar me \( U \) for eit \emph{samantrekkbart overdekke}.
\end{definition}

\begin{lemma} \label{thm:bst-good}
	For \( K \) eit endeleg abstrakt simplisielt kompleks over \( V = (v_1, \dots, v_n) \), og \( \gr{\Sd(K)}_f \) ei geometrisk realisering. Så er \( B := \set{\gr{\bst(v_i)}_f}_{i=1}^{\# V} \) eit samantrekkbart overdekke av \( \gr{\Sd(K)}_f \).
\end{lemma}

\begin{proof}
	Fyrst, merk at \( B \) er eit overdekke av \( \gr{\Sd(K)}_f \), sidan for ein vilkårlkeg \( \sigma \in \Sd(K) \), velg ein \( v_i \in \sigma_1 \), då er \( \sigma \in \bst(v_i) \). Så ein geometrisk realisering av \( \sigma \) ligg i den geometriske realiseringa av \( \bst(v_i) \).

	Merk at om \( \set{v_i, v_j} \not\in K \), så er \( \bst(v_i) \intersect \bst(v_j) = \emptyset \). Om \( \tau \in \bst(v_i) \intersect \bst(v_j) \neq \emptyset \), så ville \( \tau_1 \) ha innehaldt både \( v_i \) og \( v_j \) for at \( \tau \union \set{v_i} \in \Sd(K) \) og likt for \( v_j \). Men det betyr at \( \set{v_i, v_j} \in \bst(v_i), \bst(v_j) \) sidan dei er abstrakte simplisielle kompleks frå \autoref{thm:bst-ask}. Det betyr at \( \set{v_i, v_j} \in K \). Me kan utvida argumentet til eit vilkårleg \( \set{v_a, v_b, \dots} \)
...
\begin{proof}
	...
	Sidan \( X \) er samantrekkbart, så er \( f = \Id_X \circ f \simeq c \circ f = c \), så me har ein homotopi frå \( \Id \) til \( c \) (konstantavbildninga):
	\[
		H: S^n \times I \to X
	\]
	Der \( H(x, 0) = c \) og \( H(x, 1) = x \).

	Men det er ein avbildning \( B: S^n \times I \to D^{n+1} \) der \( (x,t) \mapsto x\dot\frac{1+t}{2} \), som dannar ein homeomorfi frå \( S^n \) til "<donuten"> mellom \( S^n \) og \( \frac{1}{2}S^n \), som me kallar for \( T^n \). Denne har invers \( B^{-1}: T^n \to S^n \), som tek \( x \mapsto (\frac{x}{||x||}, 2||x||-1) \). Merk at \( B^{-1}(\frac{1}{2}S^n)= c \), så me kan utvida \( H \circ B^{-1}: T^n \to X \) til ein avbildning:
	\[
		\hat{H \circ B^{-1}}(x) :=
		\begin{cases}
		...
		\end{cases}
	\]
	som er kontinuerlig frå \autoref{thm:pasting-lemma}.

	\( \hat{H \circ B^{-1}} \) er difor ein kontinuerlig avbildning frå \( D^{n+1} \to X \), med dei ynskja eigenskapane.
\end{proof}
...
\begin{proof}
	...
	Dette beviset er gjort ved induksjon over \( m \)-skjelettet til \( \Sd(\Nc(U)) \) for å finna ein \( H_m : \gr{\Sk_m(\Sd(\Nc(U)))}_f \times I \to \gr{\Sk_m(\Sd(\Nc(U)))}_f \) som sender \( \gr{\Sk_m(\bst(U_i))}_f \) til \( \gr{\bst(U_i)}_f \) og er ein homotopi frå \( \Id_m := \Id|_{\gr{\Sk_m(\Sd(\Nc(U)))}_f} \) til \( h_m := h|_{\gr{\Sk_m(\Sd(\Nc(U)))}_f} \).
	...
	Difor er både \( \gr{p}_f \) og \( h(\gr{p}_f) \) i ei samantrekkbar mengde \( C_p \), og me kan konstruera ein sti i \( C_p \), betegna \( s_p \), med \( s_p(0) = \gr{p}_f \) og \( s_p(1) = h(\gr{p}_f) \) mellom dei. Om me gjer dette for alle punkta så får me den fyrste homotopien:
	\[
	...
	Velg ein vilkårleg \( \sigma \in \Sk_m(\Sd(\Nc(U))) \) med \( \#\sigma = m \) (\( \sigma \) er ein \( m \)-simpleks). Og definer for \( (x, t) \in \gr{\partial\sigma}_f \times I \union \gr{\sigma}_f \times \set{0, 1} \): 
	\[
		...
		\end{cases}
	\]
	Dette er kontinuerlig frå \autoref{thm:pasting-lemma}, og \( \hat{H}_{m-1}^\sigma (x,t) \in C_\sigma \) fordi: 
	
	For \( t = 0 \), så følger det trivielt. 
	...
	Sidan frå \autoref{thm:utvida-funk} så er \( \gr{\partial\sigma}_f \times I \union \gr{\sigma}_f \times \set{0, 1} \cong S^{m} \) og \( \gr{\sigma}_f \times I \cong D^{m+1} \), så får me diagrammet og induserte avbildningar frå \autoref{thm:utvida-avb}:
